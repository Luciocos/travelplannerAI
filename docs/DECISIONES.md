...
## D-24, el viaje puede tener varias ciudades y varios países

- **Fecha:** 2026-09-23
- **Fase:** 7E
- **Contexto:** pedido explícito del usuario ("tendríamos que tener en cuenta lo de armar un itinerario para múltiples ciudades, y además múltiples países"). Hasta acá `PreferenciasViaje.destino` era un único `str`, así que un viaje a dos ciudades no se podía ni expresar.
- **Decisión:** nuevo campo `tramos: list[Tramo] | None`, con `Tramo = {destino, dias}`. `armar_plan` recorre los tramos: cada uno recupera **sus** atractivos del RAG y cobra **su** gasto diario, porque `estimar_gasto_diario` es por destino y usar un único valor para todo el viaje daría un costo falso. Los días se numeran de corrido entre tramos y las fechas siguen el calendario del viaje completo. `distribuir_dias()` respeta los días que el cliente fijó por ciudad y reparte el resto entre las demás, sin perder ni inventar días.
- **Alternativas descartadas:** (a) reemplazar `destino` por la lista y listo — descartada porque `destino` lo usan las tools que trabajan sobre una sola ciudad (`info_destino`, `buscar_alojamiento`, `buscar_vuelos`), la persistencia de chats y los tests; se conserva apuntando al primer tramo, así un viaje de una ciudad es el caso particular de un tramo único y no hay dos caminos que mantener. (b) Un plan por ciudad, concatenado en la respuesta — descartada porque el costo total y la numeración de días dejarían de ser del viaje.
- **Consecuencias:** `tramos` entra a `CAMPOS_QUE_AFECTAN_EL_PLAN`, así agregar o sacar una ciudad dispara el re-armado que ya existía. Misma convención de `None` vs `[]` que `ajustes` (D-22) para que el merge no destructivo no borre los tramos en cada turno. Cada ciudad se carga con `asegurar_destino()` (D-23), no solo la primera, o el segundo tramo se armaría sin atractivos. La tarjeta muestra la ciudad de cada día solo cuando hay más de una.

## D-25, agente de tool calling detrás de un flag

- **Fecha:** 2026-09-23
- **Fase:** 7E
- **Contexto:** el objetivo que fijó el usuario fue que el asistente "se asemeje lo más posible a charlar con un chatbot normal". Con D-22 el modelo ya redactaba, pero **qué hacer** cada turno lo seguía decidiendo `nodo_planificar` con `if`s de precondición, y las acciones posibles eran una lista cerrada. La queja concreta fue que "no extiende más ni es más flexible": no había forma de ampliar un plan ya armado. Detalle revelador: el proyecto **ya tenía las `@tool` construidas** (`crear_tool_armar_plan()` y equivalentes) con sus docstrings escritos para que un agente los leyera, y el grafo las esquivaba llamando las funciones de Python directo.
- **Decisión:** `nodo_agente_herramientas`, un loop de tool calling donde el LLM ve el catálogo y decide solo qué llamar, pudiendo encadenar varias vueltas (máximo 4). `herramientas.py` arma el catálogo con la conexión y el rotador inyectados por closure; el estado del viaje **no** se le pasa al modelo como argumento, porque son datos que el sistema ya tiene y hacérselos repetir es una invitación a que los altere. `RotadorClavesGemini.con_herramientas()` es el equivalente rotado de `bind_tools()`, con el mismo failover que el resto.
- **Por qué detrás de un flag (`AGENTE_TOOL_CALLING`):** cambia de fondo cómo se decide qué hacer en cada turno, y el camino de precondiciones ya estaba verificado a días de la defensa. Convive con él en vez de reemplazarlo: el default sigue siendo el conocido hasta que el nuevo acumule uso real.
- **Alternativas descartadas:** reemplazar el pipeline de una — descartada por riesgo; un orquestador a medio andar cerca de la entrega es peor que no tenerlo.
- **Consecuencias:** la redacción sigue siendo la de D-22, así que ningún dato duro sale del modelo: las tools devuelven los hechos y las tarjetas se arman en Python. `nodo_planificar` sigue corriendo antes del agente y es quien arma `pedir_datos`, así que el slot filling (RF1/RF2) no se tocó; si el agente pasa a default hay que decidir qué se hace con eso. Verificado en vivo contra Gemini y Postgres reales: "armame un plan de 4 días en Roma" arma el itinerario, y "dame 3 opciones más de actividades" dispara `recomendar_actividades` con la cantidad pedida, que es exactamente lo que el diseño anterior no podía hacer.
