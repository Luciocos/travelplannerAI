...
class ArgsBuscarVuelos(BaseModel):
    ...
    fecha_regreso: date | None = Field(default=None, description="Fecha de regreso, si es ida y vuelta")
...
def crear_tool_buscar_vuelos(conexion: psycopg.Connection):
    ...
    def _tool(
        origen: str, destino: str, fecha_salida: date, fecha_regreso: date | None = None, adultos: int = 1
    ) -> list[dict]:
        ...
        resultados = _buscar_vuelos(conexion, origen, destino, fecha_salida, fecha_regreso, adultos=adultos)
